...
public class ComicRepositoryTests : IDisposable
{
    ...
    internal static (ComicDbContext, SqliteConnection)
       SetupSqlite(string dbName)
    {
    ...
    {
        var lastest = await _comicRepo.GetLatestNumberAsync();
        Assert.Equal(0, lastest);
    }
