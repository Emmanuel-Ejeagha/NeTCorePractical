    ...
    for (int number = lastComic!.Number; number > 0; number--)
    {
        ...
        var continuationTask = getComicTask.ContinueWith(t =>
        {
            try
            {
                var comic = t.Result;
                if (comic != null &&
                  string.Equals(title, comic!.Title, StringComparison.OrdinalIgnoreCase))
                {
...
var results = await Parser.Default.ParseArguments<Options>(args)
    .WithParsedAsync<Options>(async options =>  // Make this async
    {
